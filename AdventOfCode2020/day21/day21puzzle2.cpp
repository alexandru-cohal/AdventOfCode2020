#include "day21puzzle2.h"

void mainDay21Puzzle2()
{
	vector<food_T> foodList;
	set<string> allergensSet;
	readInputsDay21Puzzle2(foodList, allergensSet);

	cout << solveDay21Puzzle2(foodList, allergensSet);
}
...
string solveDay21Puzzle2(vector<food_T> foodList, set<string> allergensSet)
{
	...
	// Create the dangerous ingredients canonical list 
	// Note: In the the "allergen2Ingredients" map the allergens are already sorted alphabetically
	string dangerousIngredients = "";
	for (auto itAllergen = allergen2Ingredients.begin(); itAllergen != allergen2Ingredients.end(); itAllergen++)
	{
		dangerousIngredients += *itAllergen->second.begin() + ",";
	}
	dangerousIngredients.erase(dangerousIngredients.end() - 1);
